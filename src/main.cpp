...
unsigned int nTargetSpacing = 9 * 60;
...
const string strMessageTR3B = "TR3B Signed Message:\n";
...
{
    int64_t nSubsidy = 0;
    if(nBestHeight <= 58400)
    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
    else if(nBestHeight > 58400 && nBestHeight <= 116800)
    nSubsidy = nCoinAge * COIN_YEAR_REWARD2 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 116800 && nBestHeight <= 175200)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD3 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 175200 && nBestHeight <= 233600)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD4 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 233600 && nBestHeight <= 292000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD5 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 292000 && nBestHeight <= 350400)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD6 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 350400 && nBestHeight <= 408800)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD7 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 408800 && nBestHeight <= 467200)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD8 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 467200 && nBestHeight <= 525600)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD9 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 525600 && nBestHeight <= 584000)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD10 * 33 / (365 * 33 + 8);
    else if(nBestHeight > 584000 && nBestHeight <= 642400)
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // TR3B: check proof-of-stake block signature
...
// TR3B: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TR3B", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // MainNet:
        const char* pszTimestamp = "TR3B phasing into your reality March 16, 2021";
        CTransaction txNew;
        txNew.nTime = 1615930753;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1615930753;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 30493 : 1416777;
	...
	fflush(NULL);
        ...
        assert(block.hashMerkleRoot == uint256("0x1da9348b1382024a9cf7bb5637b79ad71d2bc1d825fd1e0cbb9b5b7dccb26590"));
